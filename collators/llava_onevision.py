...
@register_collator("llava-onevision-chatas")
class LLaVAOnevisionDataCollatorChatAS(BaseDataCollator):
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        output_kwargs = self.processor._merge_kwargs(
            LlavaOnevisionProcessorKwargs,
            tokenizer_init_kwargs=self.tokenizer.init_kwargs,
        ...
        for system_prompt, cur_images, cur_videos, cur_convs in zip(system_prompts, images, videos, conversations):
            ...
            if len(cur_images) > 0:
                ...
                #sm745052 : custom truncation
                suff_sum_repeat = repeat.clone()
                for i in range(len(suff_sum_repeat)-2, -1, -1):
                    suff_sum_repeat[i] += suff_sum_repeat[i+1]

                removed_images_count = 0
                for i in range(len(suff_sum_repeat)):
                    if suff_sum_repeat[i] > max_len:
                        if cur_input_ids[0][i] == image_token_id:
                            removed_images_count += 1
                        repeat[i] = 0
                # print(removed_images_count)
                cur_images = cur_images[removed_images_count:]
                images_after_truncation.append(cur_images)
                #sm745052 : end
            ...
            # manual truncation
            if cur_input_ids.shape[1] > max_len:
                raise ValueError("sm745052 : still need to handle truncation. This shouldn't happen")
                cur_input_ids = cur_input_ids[:, max_len:]
                cur_assistant_masks = cur_assistant_masks[:, max_len:]
            cur_labels = cur_input_ids.clone()
            ...
            # padding
            # sm745052 : make it right padding during training ... # TODO : make it left padding for inference
            if cur_input_ids.shape[1] < max_len:
                cur_input_ids = torch.cat([
                    cur_input_ids,
                    torch.full(
        ...
        #sm745052 : these are the final images after truncation
        images = images_after_truncation

        # we postponed this step to get the final images after truncation
        images: List[List[PIL.Image.Image]] = [x for cur_images in images for x in cur_images]
        if len(images) > 0:
            vision_inputs.update(**self.processor.image_processor(images, return_tensors="pt", **output_kwargs["images_kwargs"]))
